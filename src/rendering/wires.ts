...
export class WireRenderer {
  ...
  private drawSiblingWire(
    ...
    const curveT = 0.5
    const indicatorX =
      (1 - curveT) * (1 - curveT) * x1 +
      2 * (1 - curveT) * curveT * controlX +
      curveT * curveT * x2
    const indicatorY =
      (1 - curveT) * (1 - curveT) * y1 +
      2 * (1 - curveT) * curveT * controlY +
      curveT * curveT * y2
  ...
  private drawDashedCurve(
    ...
    for (let i = 0; i < segments; i++) {
      ...
      const sx =
        (1 - t1) * (1 - t1) * x1 + 2 * (1 - t1) * t1 * cx + t1 * t1 * x2
      const sy =
        (1 - t1) * (1 - t1) * y1 + 2 * (1 - t1) * t1 * cy + t1 * t1 * y2
      const ex =
        (1 - t2) * (1 - t2) * x1 + 2 * (1 - t2) * t2 * cx + t2 * t2 * x2
      const ey =
        (1 - t2) * (1 - t2) * y1 + 2 * (1 - t2) * t2 * cy + t2 * t2 * y2
