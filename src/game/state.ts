...
export function getEcosystemTier(cacheTokens: number): number {
  for (let tier = TIER_THRESHOLDS.length; tier >= 1; tier--) {
    if (cacheTokens >= TIER_THRESHOLDS[tier - 1]) {
      return tier
    }
  }
  return 1
 ...
 *
 * Base chance scales with cache tokens:
 * - Starts at 25%
 * - +1% per token up to 50% softcap (25 tokens)
...
export function getCompressionChance(
  ...
  // Depth 5+ is always leaves (no compression)
  if (depth >= 5) return 0

  const tokens = cacheTokens ?? gameState.meta.cacheTokens

  // Calculate base chance with softcap/hardcap
  let baseChance = BASE_COMPRESSION_CHANCE + tokens * COMPRESSION_PER_TOKEN

  if (baseChance > COMPRESSION_SOFTCAP) {
    // Tokens beyond softcap (20 tokens)
    const tokensOverSoftcap = tokens - 20
    // Asymptotic push toward hardcap
    // Uses exponential decay: approaches 50% but never reaches it
    const push =
  ...
  // Clamp to hardcap
  baseChance = Math.min(COMPRESSION_HARDCAP, baseChance)
...
export function getBaseCompressionChance(cacheTokens?: number): number {
  ...
  if (baseChance > COMPRESSION_SOFTCAP) {
    const tokensOverSoftcap = tokens - 25
    const push =
      (COMPRESSION_HARDCAP - COMPRESSION_SOFTCAP) *
      (1 - Math.exp(-tokensOverSoftcap / 75))
    baseChance = COMPRESSION_SOFTCAP + push
