...
let siblingWires: Map<string, Wire> = new Map();

// Track packages marked as cross-package conflicted (to clear stale states)
let previouslyConflictedPackages: Set<string> = new Set();
...
export function updateCrossPackageDuplicates(): void {
  ...
  const identityToPackages = new Map<string, { packageId: string; internalId: string }[]>();
 ...
 */
export function getCrossPackageDuplicateInfo(packageId: string): { haloColor: number; identityName: string } | null {
 ...
 */
export function canCrossPackageSymlink(sourceId: string, targetId: string): string | null {
 ...
 */
export function getSharedInternalDeps(pkgAId: string, pkgBId: string): string[] {
