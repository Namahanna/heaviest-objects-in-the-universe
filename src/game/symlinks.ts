...
import { getCurrentScopePackages, getCurrentScopeWires, isInPackageScope, isCurrentScopeRoot } from './scope';
import type { Package } from './types';
import { triggerOrganizeBoost, markPackageRelocated } from './physics';

// Callback to avoid circular dependency
// Set by packages.ts on initialization
let _recalculateInternalState: ((packageId: string) => void) | null = null;

export function setRecalculateCallback(fn: (packageId: string) => void): void {
  _recalculateInternalState = fn;
}

function getRecalculateInternalState() {
  return _recalculateInternalState;
}
...
let duplicateGroups: Map<string, DuplicateGroup> = new Map();
...
export function updateDuplicateGroups(): void {
  ...
  for (const [id, pkg] of scopePackages) {
    ...
    if (!isInPackageScope() && pkg.internalPackages && pkg.internalPackages.size > 0) continue;
...
/**
 * Determine which node is "more central" for merge target selection.
 * Prefers: lower depth, then closer to origin.
 * Returns true if A is more central than B.
 */
function isMoreCentral(a: Package, b: Package): boolean {
  // Lower depth = more central (closer to root in tree)
  if (a.depth !== b.depth) {
    return a.depth < b.depth;
  }
  // Same depth: prefer closer to center (euclidean distance)
...
export function performSymlinkMerge(sourceId: string, targetId: string): number {
  ...
  // === OPTION A: Prefer inner (more central) node as target ===
  // This keeps merged hub nodes closer to center, reducing wire crossings
  if (isMoreCentral(source, target)) {
  ...
  // If in scope, also reduce the scope package's size
  if (inScope) {
    ...
    if (wire.toId === sourceId) {
      if (wire.fromId === targetId) {
        ...
        const wouldDuplicate = Array.from(wires.values()).some(
          w => w.id !== wireId && w.fromId === wire.fromId && w.toId === targetId
        );
        if (wouldDuplicate) {
          wiresToDelete.push(wireId);
        } else {
      ...
      if (wire.toId === targetId) {
        ...
        const wouldDuplicate = Array.from(wires.values()).some(
          w => w.id !== wireId && w.fromId === targetId && w.toId === wire.toId
        );
        if (wouldDuplicate) {
          wiresToDelete.push(wireId);
        } else {
  ...
  if (inScope) {
    // Recalculate internal state (might become stable)
    const recalcFn = getRecalculateInternalState();
    if (recalcFn) {
      recalcFn(gameState.currentScope);
    }
  }
 ...
 */
function markSubtreeRelocated(pkgId: string, packages: Map<string, Package>): void {
